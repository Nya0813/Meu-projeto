var path,boy,cash,diamonds,jwellery,sword;
var pathImg,boyImg,cashImg,diamondsImg,jwelleryImg,swordImg;
var treasureCollection = 0;
var cashG,diamondsG,jwelleryG,swordGroup;

/*--------------------------------------------------------*/
var PLAY = 1;
var END = 0;
var WIN = 2;
var gameState = PLAY;

var espaco,foguete 

var planG, earthG, monstG, netunoG;

var space, monster1, monster2, monster3, nave, plan1, plan2, plan3

var score=0;

var gameOver, restart;


function preload(){

space = loadImage("space.avif");
monster1 = loadImage("Monster.gif");
monster2 = loadImage("Monster2.gif");
monster3 = loadImage("Monster3.gif");
fatality = loadImage("gameOver.png");
restartImg = loadImage("resetar.png");
nave = loadImage("foguete.png");
plan1 = loadImage("saturno.png");
plan2 = loadImage("terra.png");
plan3 = loadImage("netuno.png");

}

function setup() {
   createCanvas(500,850);

   espaco = createSprite(150,200,1200,400);
   espaco.addImage("space.avif", space);
   espaco.scale = 1.5;
   espaco.velocityY = 2;
   
   foguete = createSprite(200,700,50,50);
   foguete.addImage(nave);
   foguete.scale = 0.5;

   invibleGround = createSprite(500,10,50,1800);
   invibleGround.visible = false;

   invibleGround = createSprite(0,10,50,1800);
   invibleGround.visible = false;

   invibleGround = createSprite(0,5,965,10);
   invibleGround.visible = false;

   

   planG = new Group();
   earthG = new Group();
   netunoG = new Group();
   monstG = new Group();

}

function draw() {

   if(gameState===PLAY){
   background(width/2,height/2);
   

   foguete.x = World.mouseX;
   foguete.y = World.mouseY;
      
   edges= createEdgeSprites(150,200);
   
   foguete.collide(invibleGround)
 
   if(espaco.y > 450 ){
      espaco.y = height/2;
    }

   createPlan();
   createEarth();
   createNetuno();
   createMonster();
   createMonster2();
   createMonster3();
   

   if(planG.isTouching(foguete)){
      planG.destroyEach();
      score +=5
   }

   if(earthG.isTouching(foguete)){
      earthG.destroyEach();
      score +=10
   }

   if(netunoG.isTouching(foguete)){
      netunoG.destroyEach();
      score +=3
   }

   if(monstG.isTouching(foguete)){

      gameState = END;
     monstG.destroyEach();
     netunoG.destroyEach();
     planG.destroyEach();
     earthG.destroyEach();
     foguete.destroy();
     espaco.velocityY = 0;
     
     over = createSprite(250,350,50,50);
     over.addImage(fatality);
     over.scale= 0.3;

     restart = createSprite(250,500,50,50);
     restart.addImage(restartImg);
     restart.scale = 0.3;

        
   }

   if (mousePressedOver(restart)&& gameState === END) {
      reiniciar();
    }
  
}

drawSprites();
textSize(25);
fill("white");
text("Score: "+ score, 10,30);


textSize(20);
fill("blue");
text("Don't touch the monsters!!", 200,30);

}
 
function createPlan() {
  if (World.frameCount % 200 == 0) {
  var plan= createSprite(Math.round(random(50, 250),50, 50, 50));
  plan.addImage(plan1);
  plan.scale=0.1;
  plan.velocityY = 1.5;
  plan.lifetime = 500;
  planG.add(plan);
  }
}

 
function createEarth() {
   if (World.frameCount % 200 == 0) {
   var earth= createSprite(Math.round(random(100, 50),100, 100, 100));
   earth.addImage(plan2);
   earth.scale= 0.2;
   earth.velocityY = 1;
   earth.lifetime = 500;
   earthG.add(earth);
   }
 }

  
function createNetuno() {
   if (World.frameCount % 200 == 0) {
   var netu= createSprite(Math.round(random(10, 500),10, 10, 10));
   netu.addImage(plan3);
   netu.scale= 0.2;
   netu.velocityY = 2;
   netu.lifetime = 500;
   netunoG.add(netu);
   }
 }

 function createMonster() {
   if (World.frameCount % 200 == 0) {
   var monst= createSprite(Math.round(random(10, 500),10, 10, 10));
   monst.addImage(monster1);
   monst.scale= 0.4;
   monst.velocityY = 3;
   monst.lifetime = 500;
   monstG.add(monst);
   }
 }

 function createMonster2() {
   if (World.frameCount % 200 == 0) {
   var monst= createSprite(Math.round(random(10, 500),10, 10, 10));
   monst.addImage(monster2);
   monst.scale= 0.6;
   monst.velocityY = 5;
   monst.lifetime = 500;
   monstG.add(monst);
   }
 }

 function createMonster3() {
   if (World.frameCount % 200 == 0) {
   var monst= createSprite(Math.round(random(10, 500),10, 10, 10));
   monst.addImage(monster3);
   monst.scale= 0.4;
   monst.velocityY = 6;
   monst.lifetime = 500;
   monstG.add(monst);
   }
 }

function reiniciar(){

   gameState = PLAY;
   over.visible = false;
   restart.visible = false;
   score = 0;


}